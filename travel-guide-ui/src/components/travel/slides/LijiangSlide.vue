...
const scrollToModule = (moduleId) => {
  const element = document.getElementById(`lijiang-guide-area-${moduleId}`)
  if (element) {
...
onMounted(() => {
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        const id = entry.target.id.replace('lijiang-guide-area-', '')
  ...
  nextTick(() => {
    document.querySelectorAll('.guide-module[id^="lijiang-guide-area"]').forEach(el => {
      observer.observe(el)
    })
  })
})
